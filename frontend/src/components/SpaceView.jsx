...
function Earth() {
  ...
  return <group><mesh ref={mesh}><sphereGeometry args={[DISPLAY, 64, 64]} /><meshStandardMaterial map={texture} roughness={.7} metalness={.05} emissive="#061827" emissiveIntensity={.34} /></mesh><mesh scale={1.024}><sphereGeometry args={[DISPLAY, 64, 64]} /><meshBasicMaterial color="#67ceff" transparent opacity={.14} side={THREE.BackSide} /></mesh></group>
}
...
function DebrisDot({ data }) { const ref = useRef(), rec = useMemo(() => satellite.json2satrec(data), [data]); useFrame(() => { const state = satellite.propagate(rec, new Date()); if (state?.position && ref.current) ref.current.position.set(...toScene(state.position)) }); return <mesh ref={ref}><sphereGeometry args={[.015, 4, 4]} /><meshBasicMaterial color="#ff3b3b" transparent opacity={.68} /></mesh> }
function DebrisSwarm({ catalog }) { const debris = useMemo(() => catalog.filter(x => x.OBJECT_TYPE === 'DEBRIS').slice(0, 500), [catalog]); return <group>{debris.map((x, i) => <DebrisDot key={x.NORAD_CAT_ID || i} data={x} />)}</group> }
